...
fxmpp is a Flutter plugin providing XMPP (Extensible Messaging and Presence Protocol) communication for iOS, macOS, Android, and Web. It is a fork of [haithngn/fxmpp](https://github.com/haithngn/fxmpp). The plugin exposes a stream-based Dart API. On iOS/macOS/Android it bridges to native XMPP libraries via Flutter's platform channel mechanism; on Web it speaks XMPP-over-WebSocket directly in Dart with no native/JS XMPP dependency.
...
3. Three implementations of that interface, registered per-platform in `pubspec.yaml`'s `flutter.plugin.platforms`:
   - **Method channel** (`lib/src/fxmpp_method_channel.dart`) — bridges Dart to native Android/iOS/macOS via `MethodChannel('fxmpp')` for request/response calls (connect, send, MUC operations) and `EventChannel`s for streamed data from native: `fxmpp/connection_state`, `fxmpp/messages`, `fxmpp/presence`, `fxmpp/iq`, `fxmpp/muc_events`.
   ...
   - **iOS / macOS** (`ios/fxmpp/Sources/fxmpp/FxmppPlugin.swift`, symlinked into `macos/`) — Uses [XMPPFramework](https://github.com/robbiehanson/XMPPFramework) (~> 4.0/4.1). Stanzas are sent by parsing XML strings into `DDXMLElement`. MUC admin operations (kick, ban, grant roles) are implemented as manually-constructed IQ stanzas since XMPPFramework lacks direct methods for them. The single Swift file serves both Apple platforms: a `#if canImport(FlutterMacOS)` guard selects the right Flutter import, and a `binaryMessenger(from:)` helper bridges the iOS `registrar.messenger()` method vs the macOS `registrar.messenger` property.
